 ...
 *
 * The evidence registry is a pure domain package. These tests assert the July
 * hard-stop boundary structurally: no SQL, no migrations, no hosted database
 * access, no RLS, no auth, no network, no filesystem I/O, no deployment, no UI
 * framework, no generative engine usage, no ambient clock and no process env
 * dependency.
 */

const PACKAGE_DIR = join(process.cwd(), "src/domain/evidence-registry");
...
describe("evidence registry package boundary", () => {
  ...
  it("exposes no ingestion, publishing, deletion or reinstatement API", () => {
    const barrel = readFileSync(join(PACKAGE_DIR, "index.ts"), "utf8");
    const surface = files.map((entry) => entry.text).join("\n");
    expect(barrel).not.toMatch(/ingest|publishLive|reinstate|deleteEvidence/i);
    expect(surface).not.toMatch(
      /export\s+(async\s+)?function\s+(ingest|reinstate|deleteEvidence)/i,
    );
  });
});
