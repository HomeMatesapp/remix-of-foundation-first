...
/**
 * R10. Package boundary enforcement.
 *
 * Governance is a pure domain package. These tests assert the July hard-stop
 * boundary structurally: no SQL, no migrations, no hosted database access, no
 * RLS, no auth, no live publishing, no deployment, no network, no UI framework
 * and no AI.
 */

const PACKAGE_DIR = join(process.cwd(), "src/domain/career-pack-governance");
  ...
  const walk = (dir: string) => {
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      entries.push({
        ...
        // prose that names a forbidden capability in order to forbid it must
        // not be mistaken for using it.
...
const FORBIDDEN: readonly (readonly [string, RegExp])[] = [
  ...
  ["AI", /\bAI\b|openai|anthropic|gemini|\bprompt\b|\bmodel\b|LLM/i],
];

describe("R10. governance package boundary", () => {
  ...
  it("exposes no live publishing, deletion or reinstatement API", () => {
    const barrel = readFileSync(join(PACKAGE_DIR, "index.ts"), "utf8");
    expect(barrel).not.toMatch(/publishLive|deployPack|deleteCareerPack|reinstate/i);
  });
});
